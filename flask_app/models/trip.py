...
class Trip:
    ...
    @classmethod
    def save(cls, data):
        query = 'INSERT INTO trips (departing_country, arriving_country, user_id) VALUES (%(departing_country)s, %(arriving_country)s, %(user_id)s);'
    ...
    def update_trip(cls, data):
        query = 'UPDATE trips SET departing_country=%(departing_country)s, arriving_country=%(arriving_country)s, updated_at=NOW() WHERE id=%(id)s;'
        return connectToMySQL(cls.db_name).query_db(query, data)
    ...
    def validate_inputs(trip):
        is_valid = True
        if len(trip['departing_country']) < 2:
            flash('Please select a departing Country', 'trips')
            is_valid = False
        if len(trip['arriving_country']) < 2:
